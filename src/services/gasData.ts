...
import {
  FormattedBlockData,
  FormattedBlockDataFossil,
  TWAPWindowType,
  TWAPState,
  BlockWithNextTimestamp,
  TWAPStateContainer,
  BlockWithNextTimestampFossil,
} from "../shared/types";
...
export class GasDataService {
  ...
  ): Promise<BlockWithNextTimestampFossil[]> {
    if (process.env.USE_DEMO_DATA === 'true') {
  ...
  private calculateTWAP(
    ...
    windowSize: number,
    windowStart: number,
    currentBlock: FormattedBlockDataFossil,
    relevantBlocks: BlockWithNextTimestampFossil[]
  ): TWAPState {
    let newState = { ...state };
  ...
  private async storeNewBlocks(blocks: FormattedBlockDataFossil[]): Promise<void> {
  ...
  private async getTWAPs(blockData: FormattedBlockDataFossil[]): Promise<void> {
  ...
  ): Promise<FormattedBlockDataFossil[]> {
    if (process.env.USE_DEMO_DATA === 'true') {
      return demoBlocks
